...
namespace com.IvanMurzak.Unity.MCP.Editor
{
    ...
    public static partial class Startup
    {
        ...
        public static string? ServerSourcePath
        {
            get
            {
                var localPackageDir = Path.GetFullPath(Path.Combine(Application.dataPath, "../Packages", PackageName.ToLowerInvariant()));
                if (Directory.Exists(localPackageDir))
                    return Path.GetFullPath(Path.Combine(localPackageDir, "Server"));
        ...
        public static string ServerExecutableFolder
        {
            get
            {
                var root = ServerExecutableRootPath;
                if (!File.Exists(Path.Combine(root, $"{ServerProjectName}.csproj")) && 
                    File.Exists(Path.Combine(root, "Server", $"{ServerProjectName}.csproj")))
                {
                    root = Path.Combine(root, "Server");
                }
                var net10Path = Path.Combine(root, "bin~", "Release", "net10.0");
                if (Directory.Exists(net10Path) && File.Exists(Path.Combine(net10Path, $"{ServerProjectName}.exe"))) return net10Path;
                return Path.Combine(root, "bin~", "Release", "net9.0");
            }
        }
        ...
        public static async Task BuildServer(bool force = true)
        {
            ...
            // Check if the csproj is directly in the root or in a Server subfolder
            var workingDir = ServerExecutableRootPath;
            if (!File.Exists(Path.Combine(workingDir, $"{ServerProjectName}.csproj")) && 
                File.Exists(Path.Combine(workingDir, "Server", $"{ServerProjectName}.csproj")))
            {
                workingDir = Path.Combine(workingDir, "Server");
            }

            Debug.Log($"{Consts.Log.Tag} Building server at <color=#8CFFD1>{workingDir}</color>");
